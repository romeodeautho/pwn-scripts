...
# NMAP scan against the IP list
nmapOutputBasename=nmap-3000-ports-tcp-`date '+%d-%m-%Y-%H:%M'`
sudo /usr/bin/nmap -T4 -sS -sV --script="(default or discovery) and not broadcast and not http*" --top-ports=3000 -vv -iL ips.txt -oA ${workingDir}$nmapOutputBasename
...
cat ${workingDir}ips.txt | while read line; do cat ${workingDir}domains-by-host/$line-domains.txt | while read domain; do cat ${workingDir}http-ports-by-host/$line-http-ports.txt | while read port; do echo $domain:$port >> ${workingDir}target-domains-ports-raw.txt; done ; done; done
...
cat ${workingDir}httpx.log | cut -d ' ' -f 1 | grep -v -E '^https://.*:80$' | grep -v -E '^http://.*:443$' | tee ${workingDir}httpx-urls.txt

/home/shyngys/go/bin/gowitness scan file -f ${workingDir}httpx-urls.txt --write-db --write-screenshots --write-jsonl --threads 20 --chrome-proxy http://127.0.0.1:8080
