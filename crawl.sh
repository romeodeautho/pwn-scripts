...
while getopts ":apH:C:t:h" opt; do
    case $OPTARG in
...
if ! [[ $active == '1' || $passive == '1' ]]; then echo "ERROR: At least one enumeration mode (active or passive) has to be enabled. See help: -h"; exit 1; fi
...
if [[ $active == '1' ]]; then
    ...
    cat katana-urls.txt | awk -F/ '{print $1"/"$2"/"$3"/"$4}' | sort -u | anew katana-urls-path-level-1.txt
    # search for on-site redirections
    httpx -v -l katana-urls-path-level-1.txt -sc -mc 302 -location
fi
...
if [[ $passive == '1' ]]; then
    ...
    cat gau-urless-filtered.txt urlfinder-urless-filtered.txt | awk -F/ '{print $1"/"$2"/"$3"/"$4}' | grep -E '^http' | sort -u | tee urls-paths-level-1.txt
