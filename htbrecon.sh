...
#target=$1
while getopts ":t:U:P:n:x:h" opt; do
  case $OPTARG in
...
function portscan () {
    ...
    nmap -vv -oA $nmapLogBaseName $target -Pn --disable-arp-ping
    break
    ;;
    '2') 
    echo "Performing nmap scan with service detection and default scripts..."
    nmap -sV -sC -vv -oA $nmapLogBaseName $target -Pn --disable-arp-ping
    break
    ;;
    '3') 
    echo "Performing all-ports TCP scan with OS fingerprint and default scripts..."
    nmap -sV -sC -p- -vv -oA $nmapLogBaseName $target -Pn -O --disable-arp-ping
    break
    ;;
    '4') 
    echo "Performing basic TCP + UDP scan..."
    nmap -sV -sT -sC -vv -oA $nmapLogBaseName $target -Pn --disable-arp-ping
    vared -p "Scan UDP ports?[y]" -c udpAnswer
    [[ $udpAnswer == "y" ]] && nmap -sU -sV --top-ports=100 -vv -oA $nmapLogBaseNameUDP -Pn --disable-arp-ping $target;
    break
    ;;
    '5')
    echo "Performing all-ports TCP and basic UDP scan with OS fingerprint and safe scripts..."
    nmap -p- -sT -sV --script="(default or discovery) and not broadcast and not http*" -vv -oA $nmapLogBaseName $target -Pn -O --disable-arp-ping
    vared -p "Scan UDP ports?[y]" -c udpAnswer
    [[ $udpAnswer == "y" ]] && nmap -sU -sV --top-ports=100 -vv --disable-arp-ping -oA $nmapLogBaseNameUDP -Pn $target;
    break
    ;;
    '6' )
    break
...
# creating HTML docs from Nmap output
...
function httpxRun() {
    for port in $openPorts; echo "http://${target}:${port}" | anew target-with-ports.txt;
    for port in $openPorts; echo "https://${target}:${port}" | anew target-with-ports.txt;
...
redirectionStatusCodeSearch=`awk -F ' ' -e '$2 ~ /.*30[12].*]$/' httpx.log`
if [ ! -z $redirectionStatusCodeSearch ]; then 
    echo
    echo 'Seems like HTTP redirections occured on some ports!';
    echo 'Possible new locations are:'
	awk -F ' ' -e '$2 ~ /.*30[12].*]$/ {print $6}' httpx.log | tr -d '[]';
...
if [[ -f gobuster-bruted-urls.txt ]]; then
...
while read line; do
    ...
    -u ${line}/FUZZ $replayProxyOption -o ${ffufOutputDir}/ffuf-ds-$(date '+%d-%m-%Y-%s').html -of html;" | zsh
    echo "/usr/bin/ffuf -sf -v -ic -r -w /usr/share/seclists/Discovery/Web-Content/common.txt \
    -u ${line}/FUZZ $replayProxyOption -o ${ffufOutputDir}/ffuf-common-$(date '+%d-%m-%Y-%s').html -of html;" | zsh
    ...
    -u ${line}/FUZZ/ $replayProxyOption -o ${ffufOutputDir}/ffuf-dirmed-`date '+%d-%m-%Y-%s'`.html -of html;" | zsh
    fi
done
...
while true; do
    ...
    "1") nuclei -u $target -stats -o $nucleiLogName
    break
    ;;
    "2") nuclei -u $target -etags wordpress,wp-plugin,wpscan -stats -o $nucleiLogName
    break
    ;;
    "3") nuclei -u $target -etags xss -stats -o $nucleiLogName
    break
    ;;
    "4") nuclei -u $target -etags wordpress,wp-plugin,wpscan,xss -stats -o $nucleiLogName
    break
    ;;
    "5") read -p "[*] Specify tags to include, comma separated: " tags_line
    [[ ! -z $tags_line ]] && nuclei -u $target -tags $tags_line -stats -o $nucleiLogName
    break
    ;;
    esac
done 
